...
import { clients, deadlines, jobs, recentActivity, documents } from "@/lib/mock-data";
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid } from "recharts";
import { format, differenceInDays } from "date-fns";
import { useQuery } from "@tanstack/react-query";

const chartData = [
  { name: "Jan", processed: 142, review: 18, failed: 4 },
  { name: "Feb", processed: 168, review: 22, failed: 6 },
  { name: "Mar", processed: 211, review: 31, failed: 3 },
  { name: "Apr", processed: 184, review: 24, failed: 7 },
  { name: "May", processed: 162, review: 19, failed: 2 },
  { name: "Jun", processed: 198, review: 27, failed: 5 },
];
...
export default function Dashboard() {
  ...
  const STAT_DEFS = [
    { key: "clients", label: "Active clients", value: data?.clients ?? clients.length, delta: "+2 this month", icon: Users, tone: "primary" },
    { key: "docs", label: "Documents processed", value: data?.docs ?? "1,068", delta: "+184 this week", icon: FileStack, tone: "info" },
    { key: "flagged", label: "Flagged clients", value: data?.flagged ?? clients.filter((c) => c.status === "flagged").length, delta: "Needs review", icon: AlertTriangle, tone: "destructive" },
    { key: "deadlines", label: "Upcoming deadlines (30d)", value: data?.deadlines ?? 12, delta: "3 due in 7 days", icon: CalendarClock, tone: "warning" },
  ] as const;
          ...
          <Card className="relative overflow-hidden border-border/70 bg-card p-5 shadow-elegant">
            ...
            <div className="relative">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Generated by Gemma 2 · 4 mins ago
              </p>

              <div className="mt-5 space-y-3">
                {[
                  { icon: AlertTriangle, tone: "destructive", title: "ITC mismatch — Sharma Textiles", desc: "₹48,210 of GSTR-2A credits not in your purchase register. 6 invoices affected." },
                  { icon: Zap, tone: "warning", title: "GSTR-3B due in 1 day", desc: "2 clients still haven't submitted bank statements for March 2026." },
                  { icon: TrendingUp, tone: "info", title: "Greenleaf Organics revenue +28%", desc: "Q4 GST output suggests significant Y/Y growth — flag for advisory call." },
                ].map((i) => (
